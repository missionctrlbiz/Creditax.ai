...
export async function POST(
  ...
  if (!reason || reason.length < 20) {
    return NextResponse.json(
      { success: false, message: 'Rejection reason must be at least 20 characters' },
      { status: 400 }
    );
  }
  ...
  return NextResponse.json({
    success: true,
    message: `${application.businessName} has been rejected. Notification sent to ${application.email}.`,
  });
}